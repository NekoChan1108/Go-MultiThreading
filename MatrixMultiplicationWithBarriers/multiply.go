...
var (
	...
	workStart    = NewBarrier(MatrixSize + 1) // 工作开始
	workComplete = NewBarrier(MatrixSize + 1) // 工作完成
)
...
func main() {
	...
	for i := 0; i < 100; i++ {
		...
		//condLock.Broadcast()
		// 第 i 轮
		generateRandomMatrix(&matrixA) // 主：准备数据
		generateRandomMatrix(&matrixB)

		workStart.Wait() // 主：等所有人都就位
		// ——— barrier 开闸 ———
		// worker 们：并行地算完 result

		workComplete.Wait() // 主：等所有人都跑完
		// ——— barrier 再次开闸 ———
		// 进入第 i+1 轮
	}
	fmt.Println("Done!")
